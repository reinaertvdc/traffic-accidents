const CollisionType = {
    update: () => {
      DataController.getData((data) => {
        ...
        function getCollisionTypes (data) {
          let accidents = [];
          // Parse the date / time
          ...
          for(var y = 1995; y <= 2016; y++){
            var value1 = data.trafficAccidents.filter(
                          function(val) {
                            return val.year === y && val.collisionType == 1;
                          }
                        ).length;
  
            var value2 = data.trafficAccidents.filter(
                            function(val) {
                            return val.year === y && val.collisionType == 2;
                            }
                        ).length;
  
            var value3 = data.trafficAccidents.filter(
                            function(val) {
                              return val.year === y && val.collisionType == 3;
                            }
                          ).length;
    
            var value4 = data.trafficAccidents.filter(
                              function(val) {
                              return val.year === y && val.collisionType == 4;
                              }
                          ).length;
  
            var value5 = data.trafficAccidents.filter(
                            function(val) {
                              return val.year === y && val.collisionType == 5;
                            }
                          ).length;
    
            var value6 = data.trafficAccidents.filter(
                              function(val) {
                              return val.year === y && val.collisionType == 6;
                              }
                          ).length;
  
            var value7 = data.trafficAccidents.filter(
                            function(val) {
                              return val.year === y && val.collisionType == 7;
                            }
                          ).length;
    
            var value8 = data.trafficAccidents.filter(
                              function(val) {
                              return val.year === y && val.collisionType == 8;
                              }
                          ).length;
      ...
      var counts = getCollisionTypes(data);
      console.log(counts);
      ...
      var svg = d3.select('#collisionType').append("svg")
        ...
        var categoriesNames = counts.map(function(d) { return d.year; });
        var rateNames = counts[0].values.map(function(d) { return d.type; });
  
        x0.domain(categoriesNames);
        x1.domain(rateNames).rangeRoundBands([0, x0.rangeBand()]);
        y.domain([0, d3.max(counts, function(categorie) { return d3.max(categorie.values, function(d) { return d.value; }); })]);
        ...
        var legend = svg.selectAll(".legend")
            .data(counts[0].values.map(function(d) { return d.type; }).reverse())
        .enter().append("g")
            .attr("class", "legend")
