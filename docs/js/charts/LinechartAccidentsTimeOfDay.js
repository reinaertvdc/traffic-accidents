const LinechartAccidentsTimeOfDay = {
  update: () => {
    DataController.getData((data) => {
      ...
      // Adds the svg canvas
      var svg = d3.select("#linechartAccidentsTimeOfDay")
          .append("svg")
      ...
      x.domain(d3.extent(weekdaysCounts, function(d) { return d.hour; }));
      y.domain([0, d3.max(weekdaysCounts, function(d) { return d.value; })]);
...
DataController.addObserver(() => {
  LinechartAccidentsTimeOfDay.update();
});
